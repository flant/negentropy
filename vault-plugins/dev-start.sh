...
function activate_plugin() {
  ...
  if [ $plugin == "flant_iam" ]; then
    docker-compose exec -T vault sh -c "vault write -force $plugin/kafka/generate_csr" >/dev/null 2>&1
    docker-compose exec -T vault sh -c "vault write $plugin/kafka/configure_access kafka_endpoints=kafka:9092"
