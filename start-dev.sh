...
function docker-exec() {
  docker-compose -f docker/docker-compose.common.yml -f docker/docker-compose.dev.yml exec -e VAULT_TOKEN=${VAULT_TOKEN:-root} -T vault sh -c "$@"
}
...
function connect_plugins() {
 ...
 # configure flant_iam_auth
  docker-exec "vault write auth/flant_iam_auth/kafka/configure peers_public_keys=\"$root_pubkey\" self_topic_name=auth-source.auth-1 root_topic_name=root_source.auth-1 root_public_key=\"$root_pubkey\""
}

function initialize() {
  # configure server_access
...
EOF'
  ...
  # renew root token
  docker-exec "vault token create -orphan -policy=root -field=token" > /tmp/vault_dev_token
  export VAULT_TOKEN="$(cat /tmp/vault_dev_token)"
