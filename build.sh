...
function build_plugin() {
  ...
  mkdir -p $TMP_DIR/vault-plugins-build
  ...
  docker run --rm \
    ...
    -v $TMP_DIR/vault-plugins-build:/go/pkg/mod \
    $EXTRA_MOUNT \
    -e CGO_ENABLED=1 \
...
function build_authd() {
  ...
  mkdir -p $TMP_DIR/authd-build

  docker run --rm \
    -w /go/src/app/authd \
    -v $AUTHD_DIR/build:/src/build \
    -v $AUTHD_DIR:/go/src/app/authd \
    -v $TMP_DIR/authd-build:/go/pkg/mod \
    -e GO111MODULE=on \
    golang:1.16 \
...
function build_cli() {
  ...
  docker run --rm \
    ...
    -v $TMP_DIR/cli-build:/go/pkg/mod \
    -e GO111MODULE=on \
    golang:1.16 \
...
function build_server_accessd() {
  ...
  mkdir -p $TMP_DIR/server-accessd-build
  ...
  docker run --rm \
    ...
    -v $TMP_DIR/server-accessd-build:/go/pkg/mod \
    -e GO111MODULE=on \
    golang:1.16 \
...
function build_oidc_mock() {
  ...
  mkdir -p $TMP_DIR/oidc-mock-build

  docker run --rm \
    -w /go/src/oidc-mock \
    -v $OIDC_MOCK_DIR/build:/src/build \
    -v $OIDC_MOCK_DIR:/go/src/oidc-mock \
    -v $TMP_DIR/oidc-mock-build:/go/pkg/mod \
    golang:1.16-alpine \
