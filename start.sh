...
EOF'
  ...
  # renew root token
  docker-exec "vault_root" "vault token create -orphan -policy=root -field=token" > /tmp/root_token
  docker-exec "vault_auth" "vault token create -orphan -policy=root -field=token" > /tmp/auth_token
