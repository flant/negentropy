...
def upgrade(vault_name: str, vaults: List[Vault]):
    ...
    for plugin in plugins:
        ...
        if plugin == 'flant_iam_auth':
            vault_client.write(path='auth/flant/configure_extension/server_access',
                               role_for_ssh_access='ssh.open')
        elif plugin == 'flant_iam':
            vault_client.write(path='flant/configure_extension/server_access', roles_for_servers=["server"],
