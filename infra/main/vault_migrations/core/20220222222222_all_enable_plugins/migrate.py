...
auth_vaut_plugins = {'ssh': 'secret', 'flant_iam_auth': 'auth'}
    ...
    for plugin_name, plugin_type in plugins_to_activate.items():
        if plugin_type == 'secret':
            if plugin_name not in active_secrets:
                ...
                print("secret plugin '{}' is activated at '{}' vault".format(plugin_name, vault_name))
            else:
                print("secret plugin '{}' already activated at '{}' vault".format(plugin_name, vault_name))
        if plugin_type == 'auth':
            if 'auth/' + plugin_name not in active_auths:
                vault_client.sys.enable_auth_method(
                    method_type=plugin_name,
                    path=plugin_name,
                    plugin_name=plugin_name,
                )
                print("auth plugin '{}' is activated at '{}' vault".format(plugin_name, vault_name))
            else:
                print("auth plugin '{}' already activated at '{}' vault".format(plugin_name, vault_name))
