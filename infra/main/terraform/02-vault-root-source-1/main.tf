...
resource "google_compute_instance" "vault-root-source-1" {
  zone         = "europe-west1-c"
  ...
  hostname                  = "${local.prefix}-vault-root-source-1.negentropy.flant.local"
  allow_stopping_for_update = true
  metadata = {
    block-project-ssh-keys = "TRUE"
  }
  tags = ["${local.prefix}-vault-root-source-1"]
  service_account {
...
resource "google_compute_firewall" "vault-root-source-1" {
  name    = "${local.prefix}-vault-root-source-1"
  ...
  target_tags = ["${local.prefix}-vault-root-source-1"]
}
