...
locals {
  ...
  buckets = distinct([for i in local.instances : i.bucket])
  ...
  common = {
    ...
    "auth_instance_group" : false
    "root_source_instance_group" : false
    "ssh_user" : "user"
  ...
  instances = [
    ...
    merge(local.common,
      {
        "name" : join("-", [var.prefix, "vault-auth-ew3a1"])
        "region" : "europe-west3"
        "zone_postfix" : "a"
        ...
        "auth_instance_group" : "true"
        "order_public_static_ip" : "true"
    ...
    merge(local.common,
      {
        ...
        "root_source_instance_group" : "true"
