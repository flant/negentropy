...
resource "google_compute_disk" "kafka-1" {
  zone     = "europe-west1-c"
  name     = "${local.prefix}-kafka-data-1"
  type     = "pd-ssd"
  size     = "30"
}
...
resource "google_compute_instance" "kafka-1" {
  zone         = "europe-west1-c"
  name         = "${local.prefix}-kafka-1"
  ...
  attached_disk {
    source      = google_compute_disk.kafka-1.self_link
    device_name = google_compute_disk.kafka-1.name
  }
  network_interface {
  ...
  hostname                  = "${local.prefix}-kafka-1.negentropy.flant.local"
  allow_stopping_for_update = true
  metadata = {
    block-project-ssh-keys = "TRUE"
  }
  tags = ["${local.prefix}-kafka-1"]
  service_account {
...
resource "google_compute_firewall" "kafka-1" {
  name    = "${local.prefix}-kafka-1"
  ...
  target_tags = ["${local.prefix}-kafka-1"]
}

resource "google_dns_record_set" "kafka-1" {
  name         = "${local.prefix}-kafka-1.negentropy.flant.local."
  type         = "A"
  ttl          = 300
...
resource "google_dns_record_set" "kafka-1-ptr" {
  name         = "${local.private_ptr}.in-addr.arpa."
  type         = "PTR"
  ttl          = 300
  managed_zone = "ptr"
  rrdatas      = ["${local.prefix}-kafka-1.negentropy.flant.local."]
}
