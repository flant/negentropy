...
variable "vault_auth_internal_domain" {
  type = string
}

variable "vault_auth_external_domain" {
  type = string
}
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "VAULT_BUCKET=${var.vault_auth_bucket}",
  ...
  provisioner "shell" {
    ...
    scripts         = [
      ...
      "packer-scripts/01-vault-addr.sh",
      "packer-scripts/02-uncomment-vars.sh",
      "packer-scripts/03-nginx.sh"
    ]
  }
  ...
  provisioner "file" {
    sources     = [
      ...
      "config/nginx-vault-external.conf",
      "config/nginx-vault-healthcheck.conf"
    ]
