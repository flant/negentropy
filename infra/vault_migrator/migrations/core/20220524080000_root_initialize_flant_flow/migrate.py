...
def upgrade(vault_name: str, vaults: List[Vault]):
    ...
    cfg = vault.client.read(path='flant/configure_extension/flant_flow').get('data').get('flant_flow_cfg')
    ...
    if not flant or flant == '':
        ...
        vault.client.write(path='flant/configure_extension/flant_flow/flant_tenant/' + flant_tenant_uuid)
    ...
    if not all_flant_group or all_flant_group == '':
        vault.client.write(path='flant/configure_extension/flant_flow/all_flant_group/' + all_flant_group_uuid)
    ...
    if not all_flant_group_rolebinding_uuid or all_flant_group_rolebinding_uuid == '':
        vault.client.write(path='flant/configure_extension/flant_flow/all_flant_group_roles', roles=['flant.teammate'])
    ...
    if not spec or not spec.get("devops_service_pack"):
        vault.client.write(path='flant/configure_extension/flant_flow/service_packs_roles_specification',
    ...
    if not teams.get(l1_team_name):
        vault.client.write(path='flant/team/privileged', uuid=l1_team_uuid, identifier=l1_team_id,
                           team_type='standard_team')
        vault.client.write(path='flant/configure_extension/flant_flow/specific_teams',
    ...
    if not teams.get(mk8s_team_name):
        vault.client.write(path='flant/team/privileged', uuid=mk8s_team_uuid, identifier=mk8s_team_id,
                           team_type='standard_team')
        vault.client.write(path='flant/configure_extension/flant_flow/specific_teams',
    ...
    if not teams.get(okmeter_team_name):
        vault.client.write(path='flant/team/privileged', uuid=okmeter_team_uuid, identifier=okmeter_team_id,
                           team_type='standard_team')
        vault.client.write(path='flant/configure_extension/flant_flow/specific_teams',
    ...
    if not primary_administrators_roles or len(primary_administrators_roles) == 0:
        vault.client.write(path='flant/configure_extension/flant_flow/client_primary_administrators_roles',
