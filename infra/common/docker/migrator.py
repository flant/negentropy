...
def upgrade_vaults(vaults: List[VaultParams], migration_dir: str, migration_config_path: str, version: str = None):
    """
    ...
    :param migration_config_path: full path to migration config file with values for environment variables
    :param version: valid UTC timestamp, the last operated migration will not exceed, example: 20210716203309
    :return:
    """
    print("start migration with '{}' config file".format(migration_config_path))
    migration_config = list(yaml.safe_load_all(open(migration_config_path, "r")))
    cfg = next(i for i in migration_config if i["kind"] == "MigrationConfiguration")
